...
#1014
def perv_clips_menu():
	process.Menu('Most Popular','https://pervclips.com/tube/most-popular/',1015,'',FANART,'','')
	process.Menu('Popular','https://pervclips.com/tube/',1015,'',FANART,'','')
	process.Menu('Recently Added','https://pervclips.com/tube/latest-updates/',1015,'',FANART,'','')
	process.Menu('Top Rated','https://pervclips.com/tube/top-rated/',1015,'',FANART,'','')
	process.Menu('Categories','https://pervclips.com/tube/categories/videos/',1017,'',FANART,'','')
	process.Menu('Search','',1018,'',FANART,'','')
...
def pervclips_vids(url):
	...
	match = re.compile('itemprop="url" href="(.+?)" class="img".+?src="(.+?)".+?alt="(.+?)".+?itemprop="duration">(.+?)</span>',re.DOTALL).findall(html)
...
def pervclips_cats(url):
	...
	for url,img,name,vid_count in match:
		for alt_imgs in alt_img:
			if  '.gif' in img:
				img = img.replace(img,alt_imgs)
